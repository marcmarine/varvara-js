...
import { DefaultThemeRenderContext, JSX, PageEvent, Reflection, type RenderTemplate } from 'typedoc'
...
export function layout(context: VarvaraThemeContext) {
  return (template: RenderTemplate<PageEvent<Reflection>>, props: PageEvent<Reflection>) => (
    <html class="default" lang={context.options.getValue('lang')} data-base={context.relativeURL('./')}>
      <head>
        ...
        <title>{props.model.isProject() ? getDisplayName(props.model) : `${getDisplayName(props.model)} | ${getDisplayName(props.project)}`}</title>
        {favicon(context)}
        ...
        {!!getHierarchyRoots(props.project).length && <script async src={context.relativeURL('assets/hierarchy.js', true)} id="tsd-hierarchy-script"></script>}
        {context.hook('head.end', context)}
        {context.options.getValue('customCss') && <link rel="stylesheet" href={context.relativeURL('assets/custom.css', true)} />}
        {context.options.getValue('customJs') && <script defer src={context.relativeURL('assets/custom.js', true)}></script>}
      </head>
      <body>
